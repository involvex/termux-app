...
import com.invapp.shared.reflection.ReflectionUtils;
...
import com.invapp.shared.termux.TermuxConstants.TERMUX_APP;
...
public class TermuxUtils {
     ...
     *
     * If your third-party app is targeting sdk `30` (android `11`), then it needs to add `com.invapp`
     * package to the `queries` element or request `QUERY_ALL_PACKAGES` permission in its
     * `AndroidManifest.xml`. Otherwise it will get `PackageSetting{...... com.invapp/......} BLOCKED`
     ...
     * <manifest
     *     <queries>
     *         <package android:name="com.invapp" />
     *    </queries>
     * </manifest>
     * }
     *
    ...
    public static String geAPTInfoMarkdownString(@NonNull final Context context) {
        ...
        InputStream inputStream = context.getResources().openRawResource(com.invapp.shared.R.raw.apt_info_script);
        try {
